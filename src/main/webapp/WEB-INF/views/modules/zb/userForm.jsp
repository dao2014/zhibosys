...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/zb/user/">获取用户成功列表</a></li>
		<li class="active"><a href="${ctx}/zb/user/form?id=${user.id}">获取用户成功<shiro:hasPermission name="zb:user:edit">${not empty user.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="zb:user:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/zb/user/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			<label class="control-label">wacht_name：</label>
			<div class="controls">
				<form:input path="wachtName" htmlEscape="false" maxlength="100" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">wacht_open_id：</label>
			<div class="controls">
		...
		<div class="form-actions">
			<shiro:hasPermission name="zb:user:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
